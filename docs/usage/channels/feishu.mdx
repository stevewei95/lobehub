  ...
  <Tabs items={['Feishu (飞书)', 'Lark (International)']}>
    ...
    <Tab>
      ...
      <Callout type={'warning'}>
        Lark (international) does not support some Feishu-specific scopes such as `im:chat.access_event.bot_p2p_chat:read` and `im:chat.members:bot_access`. The JSON above uses Lark-compatible equivalents.
      </Callout>
    </Tab>
  </Tabs>
